...
bool Map::Frame()
{
	T::Vector3 vLight(cosf(g_fGameTimer) * 100.0f, 100, sinf(g_fGameTimer) * 100.0f);
...
bool Map::SetVertexData()
{
	...
	for (int iRow = 0; iRow < m_iNumRows; iRow++)
	{
		for (int iCol = 0; iCol < m_iNumCols; iCol++)
		{
			...
			m_VertexList[index].n = T::Vector3(0, 1, 0);
			m_VertexList[index].c = T::Vector4(randstep(0.0f, 1.0f), 
				randstep(0.0f, 1.0f), 
				randstep(0.0f, 1.0f), 1);
			m_VertexList[index].t = 
				T::Vector2(ftxOffetU*iCol, ftxOffetV * iRow);
		}
	}
	return true;
}
bool Map::SetIndexData()
{
	...
	for (int iRow = 0; iRow < m_iNumCellRows; iRow++)
	{
		for (int iCol = 0; iCol < m_iNumCellCols; iCol++)
		{
			...
			T::Vector3 vNormal;
			T::Vector3 vE0= m_VertexList[face.v1].p - m_VertexList[face.v0].p;
			T::D3DXVec3Normalize(&vE0, &vE0);
			T::Vector3 vE1= m_VertexList[face.v2].p - m_VertexList[face.v0].p;
			...
			m_VertexList[face.v0].c = T::Vector4(fDot, fDot, fDot,1);
			m_VertexList[face.v1].c = T::Vector4(fDot, fDot, fDot, 1);
			m_VertexList[face.v2].c = T::Vector4(fDot, fDot, fDot, 1);
	...
	for (int iRow = 0; iRow < m_iNumRows; iRow++)
	{
		for (int iCol = 0; iCol < m_iNumCols; iCol++)
		{
			...
			m_VertexList[iRow * m_iNumCols + iCol].c = T::Vector4(fDot, fDot, fDot, 1);
		}
	}
	return true;
}
