...
{	
	...
	m_CameraTopView.CreateViewMatrix(T::Vector3(0, 3000.0f, -1), T::Vector3(0, 0.0f, 0));
	m_CameraTopView.CreateProjMatrix(XM_PI * 0.25f, (float)g_rtClient.right / (float)g_rtClient.bottom, 1.0f, 30000.0f);

	m_Camera.Init();
	m_Camera.CreateViewMatrix(T::Vector3(0, 300.0f, -100.0f), T::Vector3(0, 0.0f, 0));
	...
	m_Camera.SetPosition(T::Vector3(0.0f, 1.0f, 0.0f));
	...
	m_PlayerObj.SetPosition(T::Vector3(0.0f, 1.0f, 0.0f));
	...
	m_SkyObj.Init();	
	m_SkyObj.SetPosition(T::Vector3(0.0f, 0.0f, 0.0f));	
	...
	for (int iObj = 0; iObj < m_ObjList.size(); iObj++)
	{
		...
		m_ObjList[iObj].SetPosition(T::Vector3(-300.0f + rand() % 600, 100.0f, -300.0f + rand() % 600));
...
bool Sample::Frame()
{	
	T::Vector2 dir = Input::Get().GetDelta();
	...
	m_Camera.Update(T::Vector4(-dir.x, -dir.y,0,0));
	m_MapObj.Frame();
	m_PlayerObj.Frame();

	T::Matrix matRotateObj;
	for (auto& obj : m_ObjList)
	{
		T::D3DXMatrixScaling(&matScale, 10 * cosf(g_fGameTimer), 10 * cosf(g_fGameTimer), 10 * cosf(g_fGameTimer));
		T::D3DXMatrixRotationYawPitchRoll(&matRotateObj, cosf(g_fGameTimer * obj.m_vPos.x * 0.001f) * XM_PI, sinf(g_fGameTimer * obj.m_vPos.y * 0.001f) * XM_PI, 1.0f);
...
{	
	...
	for (auto& obj : m_ObjList)
	{
		...
		{
			obj.SetMatrix(nullptr, &m_CameraTopView.m_matView, &m_CameraTopView.m_matProj);
			obj.m_ConstantList.Color = T::Vector4(0, 0, 0, 1);
			obj.Render();
		}
	}
