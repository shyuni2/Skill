...
void SDxRT::Set(ID3D11Device* pDevice, FLOAT TopLeftX, FLOAT TopLeftY, FLOAT Width, FLOAT Height, FLOAT MinDepth, FLOAT MaxDepth)
{
...
void SDxRT::Apply(ID3D11DeviceContext* pImmediateContext, ID3D11RenderTargetView* pRTV, ID3D11DepthStencilView* pDepthStencilView, bool bDepthClear, bool bStencilClear)
{
...
bool SDxRT::Begin(ID3D11DeviceContext* pContext, SVector4 vColor, bool bTargetClear, bool bDepthClear, bool bStencilClear)
{
	m_nViewPorts = 1;
...
bool SDxRT::Clear(ID3D11DeviceContext* pContext, SVector4 vColor,
...
HRESULT SDxRT::SaveFile(ID3D11DeviceContext* pContext, T_STR name)
{
	...
	if (m_pTexture != nullptr)
	{
		if (STextureMgr::SaveFile(pContext, m_pTexture.Get(), name))
		{
			return hr;
		}
	}
	else if (m_pSRV != nullptr)
	{
		...
		hr = STextureMgr::SaveFile(pContext, texture, name);
		texture->Release();
	}
	else if (m_pRenderTargetView != nullptr)
	{
		...
		hr = STextureMgr::SaveFile(pContext, texture.Get(), name + L"RTV");
	}
	return hr;
}
