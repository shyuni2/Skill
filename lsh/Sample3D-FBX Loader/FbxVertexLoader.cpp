...
FbxVector4 FbxLoader::ReadNormal(const FbxMesh* mesh, int controlPointIndex, int vertexCounter)
{
	...
	{
	case FbxGeometryElement::eByControlPoint: // 제어점 마다 1개의 매핑 좌표가 있다.
	{
	...
	case FbxGeometryElement::eByPolygonVertex: // 정점 마다 1개의 매핑 좌표가 있다
	{
		switch (vertexNormal->GetReferenceMode())
		{
		case FbxGeometryElement::eDirect:
...
std::string FbxLoader::ParseMaterial(FbxSurfaceMaterial* pMtrl)
{
	...
	if (Property.IsValid())
	{
		...
		if (tex != nullptr)
		{
			...
			std::string ext = Ext;
			if (ext == ".tga" || ext == ".TGA") // tga 확장자 파일을 dds로 변경
			{
				ext.clear();
				ext = ".dds";
			}
