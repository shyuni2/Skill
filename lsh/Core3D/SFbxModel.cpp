...
bool SFbxModel::CreateVertexBuffer()
{
	...
	for (int iMtrl = 0; iMtrl < m_pSubVertexList.size(); iMtrl++)
	{
		...
		if (FAILED(hr = m_pd3dDevice->CreateBuffer(&bd, &sd, &m_pVBList[iMtrl]))) return false;
	}
	for (int iMtrl = 0; iMtrl < m_pSubIWVertexList.size(); iMtrl++)
	{
		...
		if (FAILED(hr = m_pd3dDevice->CreateBuffer(&bd, &sd, &m_pVBWeightList[iMtrl]))) return false;
	}
	return true;
}
bool SFbxModel::SetIndexData()
{	
	return true;
}
bool SFbxModel::CreateInputLayout()
{
