...
ID3D11DepthStencilState* g_pDSSDepthEnableWriteDisable = nullptr;
ID3D11DepthStencilState* g_pDSSDepthDisableWriteDisable = nullptr;
...
bool DxState::SetState(ID3D11Device* pd3dDevice)
{
	...
	if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &DxState::g_pRSBackCullSolid)))
	{
		return hr;
	}
	...
	if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &DxState::g_pRSNoneCullSolid)))
	{
		return hr;
	}
	...
	rsDesc.CullMode = D3D11_CULL_BACK;
	if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &DxState::g_pRSBackCullWireFrame)))
	{
		return hr;
	}
	...
	rsDesc.FillMode = D3D11_FILL_WIREFRAME;
	rsDesc.CullMode = D3D11_CULL_NONE;
	if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &DxState::g_pRSNoneCullWireFrame)))
	{
		return hr;
	}
