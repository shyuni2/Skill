...
ID3D11BlendState* SDxState::m_AlphaBlend = nullptr;
ID3D11BlendState* SDxState::m_AlphaBlendDisable = nullptr;
ID3D11BlendState* SDxState::m_BSNoneColor = nullptr;

ID3D11SamplerState* SDxState::m_pSSLinear = nullptr;
ID3D11SamplerState* SDxState::m_pSSPoint = nullptr;
...
ID3D11DepthStencilState* SDxState::g_pDSSDepthEnableWriteDisable = nullptr;
ID3D11DepthStencilState* SDxState::g_pDSSDepthDisableWriteDisable = nullptr;

bool SDxState::SetState(ID3D11Device* pd3dDevice)
{
	HRESULT hr;
	...
	rsDesc.CullMode = D3D11_CULL_BACK;
	if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &SDxState::g_pRSBackCullWireFrame))) 
	{
		return hr;
	}
	// 와이어프레임 x
	...
	rsDesc.FillMode = D3D11_FILL_WIREFRAME;
	rsDesc.CullMode = D3D11_CULL_NONE;
	if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &SDxState::g_pRSNoneCullWireFrame))) 
	{
		return hr;
	}
	rsDesc.FillMode = D3D11_FILL_SOLID;
