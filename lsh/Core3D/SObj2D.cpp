...
void SObj2D::SetRectSouce(RECT rt)
{
	m_rtSource = rt;
}
void SObj2D::SetRectDraw(RECT rt)
{
	m_rtDraw = rt;
...
void SObj2D::UpdateRectDraw(RECT rt)
{	
	m_fWidth = rt.right;
	m_fHeight = rt.bottom;
}
void SObj2D::AddPosition(S::SVector2 vPos)
{
	// 현재위치
	m_vPos += vPos;
	//  10,10f    ~     w(50, 10)
...
void SObj2D::SetPosition(S::SVector2 vPos)
{
	m_vPos = vPos;	
	m_rtCollision = Rect(m_vPos, m_fWidth, m_fHeight);	
	SetVertexData();
...
void SObj2D::Convert(S::SVector2 center, float fWidth, float fHeight, std::vector<SimpleVertex>& retList)
{
	// 0       1,4
...
void SObj2D::Convert(std::vector<SimpleVertex>& list, std::vector<SimpleVertex>& retList)
{
...
void SObj2D::ConvertIndex(S::SVector2 center, float fWidth, float fHeight, std::vector<SimpleVertex>& retList)
{
	// 0       1
...
void SObj2D::ConvertIndex(std::vector<SimpleVertex>& list, std::vector<SimpleVertex>& retList)
{
	retList.resize(list.size());
	for (int i = 0; i < list.size(); i++)
	{
