...
namespace Math
{
	...
	void Matrix::Translation(const Math::Vector3& v)
	{
		_41 = v.x;
		_42 = v.y;
		_43 = v.z;
	...
	void Matrix::Scale(const Math::Vector3& vector)
	{
		_11 = vector.x;
		_22 = vector.y;
		_33 = vector.z;
	...
	Matrix Matrix::ViewLookAt(Math::Vector3& vPosition, Math::Vector3& vTarget, Math::Vector3& vUp)
	{
		Matrix matrix;
		Math::Vector3 vDirection = (vTarget - vPosition).Normal();		// Z Axis
		Math::Vector3 vRightVector = (vUp ^ vDirection).Normal();		// X Axis
		Math::Vector3 vUpVector = (vDirection ^ vRightVector).Normal();	// Y Axis
	...
	Matrix Matrix::CreateViewLook(Math::Vector3& vPosition, Math::Vector3& vTarget, Math::Vector3& vUp)
	{
		Matrix matrix;
		Math::Vector3 vDirection = vTarget - vPosition;
		vDirection = vDirection.Normal();//z
		float fDot = vUp | vDirection;
		// 직교의 근사화 작업
		Math::Vector3 vUpVector = vUp - (vDirection * fDot);
		vUpVector = vUpVector.Normal();
		Math::Vector3 vRightVector = vUpVector ^ vDirection;
	...
	Matrix Matrix::RotationViewYawPitchRoll(float yaw, float pitch, float roll)
	{
		Matrix matY;
		matY.YRotate(yaw);
		Matrix matX;
