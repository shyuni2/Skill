...
void Obj2D::AddPosition(T::TVector2 vPos)
{
	// 현재위치
	m_vPos += vPos;
	//  10,10f    ~     w(50, 10)
...
void Obj2D::SetPosition(T::TVector2 vPos)
{
	m_vPos = vPos;	
	m_rtCollision = Rect(m_vPos, m_fWidth, m_fHeight);	
	SetVertexData();
...
void Obj2D::Convert(T::TVector2 center, float fWidth, float fHeight, std::vector<SimpleVertex>& retList)
{
	// 0       1,4
...
void Obj2D::ConvertIndex(T::TVector2 center, float fWidth, float fHeight, std::vector<SimpleVertex>& retList)
{
	// 0       1
