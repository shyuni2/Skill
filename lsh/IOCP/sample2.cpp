...
HANDLE g_hKillEvent; // 이 이벤트가 발동하면 스레드 끝내기, 정상적으로 종료했을때 이벤트랑 오류났을때 이벤트 나누기
...
DWORD WINAPI WorkerThread(LPVOID param)
{
	...
	while (1)
	{
		...
		// 완료 큐에 데이터가 있으면 작업시작(리턴), 하나도 없으면 그냥 대기
...
void main()
{
	...
	g_hFileRead = CreateFile(L"test.txt", GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, NULL); // 파일생성
	if (g_hFileRead == INVALID_HANDLE_VALUE)
	{
		return;
	}
	g_hFileWrite = CreateFile(L"copy.txt", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, NULL); // 파일생성
