...
bool Sample::Init()
{
	...
	m_Camera.CreateViewMatrix(TVector3(0, 10, -100), TVector3(0, 0, 0));
	...
	m_MapObj.Create(L"../../shader/ProjectionShadow.hlsl", L"../../shader/ProjectionShadow.hlsl", L"../../data/grass.bmp");
	...
	PSBlob = TModel::LoadShaderBlob(L"../../shader/CharacterShader.hlsl", "PSShadow", "ps_5_0");
	if (PSBlob != nullptr)
	{
		HRESULT hr = S_OK;
	...
	PSBlob = TModel::LoadShaderBlob(L"../../shader/ProjectionShadow.hlsl", "PSDepth", "ps_5_0");
	if (PSBlob != nullptr)
	{
		HRESULT hr = S_OK;
		hr = g_pd3dDevice->CreatePixelShader(PSBlob->GetBufferPointer(), PSBlob->GetBufferSize(), NULL, &m_pPSShadowMap);
		if (FAILED(hr)) return hr;
		PSBlob->Release();
	}
	CStopwatch t;
	m_FbxCharacter.LoadObject("../../data/Man2.fbx", "../../shader/CharacterShader.hlsl");
	t.Output(L"");
...
bool Sample::Render()
{
	...
	if (m_Rt.Begin(m_pImmediateContext))
	{
		...
		m_MapObj.PostRender(m_pImmediateContext, m_MapObj.m_iNumIndex);
	...
	m_pImmediateContext->UpdateSubresource(m_pShadowCB, 0, NULL, &m_ShadowCB, 0, 0);
	...
	D3DXMatrixInverse(&m_MapObj.m_cbData.matNormal, NULL, &m_MapObj.m_matWorld);;
	...
	D3DXMatrixInverse(&m_FbxCharacter.m_cbData.matNormal, NULL, &m_FbxCharacter.m_matWorld);
	m_FbxCharacter.SetMatrix(&m_FbxCharacter.m_matWorld, &m_Camera.m_matView, &m_Camera.m_matProj);
	m_FbxCharacter.SetPixelShader(nullptr, &m_FbxCharacter.m_cbData.matNormal, m_Light1.m_vDir);
	m_pImmediateContext->PSSetShaderResources(1, 1, &m_Rt.m_pTextureSRV);
	m_FbxCharacter.Render(m_pImmediateContext);
