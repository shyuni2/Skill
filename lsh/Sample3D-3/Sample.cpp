...
void    Sample::CreateMapObject()
{
	...
	for (int iObj = 0; iObj < MAX_NUM_OBJECTS; iObj++)
	{
		...
		S::D3DXMatrixScaling(&matScale, randstep(10.0f, 100.0f),
...
{	
	...
	m_CameraTopView.CreateViewMatrix(S::SVector3(0, 3000.0f, -1),
									 S::SVector3(0, 0.0f, 0));
	m_CameraTopView.CreateProjMatrix(XM_PI * 0.25f,
		(float)g_rtClient.right / (float)g_rtClient.bottom, 1.0f, 30000.0f);

	m_Camera.Init();
	m_Camera.CreateViewMatrix(  S::SVector3(0, 2000.0f, -100.0f),
								S::SVector3(0, 0.0f, 0));
...
bool	Sample::Frame()
{	
	S::SVector2 dir = TInput::Get().GetDelta();
...
bool    Sample::MiniMapRender()
{
	/*for (int iNode = 0; iNode < m_Quadtree.g_pDrawLeafNodes.size(); iNode++)
	{
			...
			m_MapObj.m_ConstantList.Color = S::SVector4(0, 1, 0, 1);
	...
	for (auto obj : m_pObjList)
	{
		obj->pObject->SetMatrix(&obj->matWorld, &m_CameraTopView.m_matView,
				&m_CameraTopView.m_matProj);
		obj->pObject->m_ConstantList.Color = S::SVector4(0, 0, 0, 1);
		obj->pObject->Render();
	}
