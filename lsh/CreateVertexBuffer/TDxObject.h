...
public:
	...
	bool Create(ID3D11Device* m_pd3dDevice, ID3D11DeviceContext* m_pContext, SVector2 pos, float fWidth, float fHeight);
	// 화면좌표 위치를 중점으로 NDC변환하는 컨버터함수
	void Convert(SVector2 center, float fWidth, float fHeight, vector<SimpleVertex>& retList);
	// 화면좌표계를 NDC로 변환
